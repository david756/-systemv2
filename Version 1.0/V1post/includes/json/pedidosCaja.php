  ...
  <?php 
          ...
          $query = "SELECT 
                ...
                WHERE (ap.hora_pedido)<(DATE_SUB(NOW(), INTERVAL 0 hour)) and (ap.hora_pedido)>(DATE_SUB(NOW(), INTERVAL 10 hour))
        ...
        while($rows=mysql_fetch_array($consulta)){ 
           ...
           $texto = '<tr> 
                      <td>'.$rows[0].'</td> 
                      <td>'.($rows[1]-$rows[5]).'</td>
                      <td><a href="pago.php?atencion='.$rows[2].' "><button> '.$accion.' </button></a></td>
                      <td>'.$rows[4].'</td>
