   ...
   function Producto($idProducto="def", $nombre="def", $valor="def", $descripcion="def", $categoria="def") {
    ...
    function getProductos() {
     ...
     function productoAtenciones(){         
        ...
        $estado=true;
        $pdo = Database::connect();
        $query = "SELECT * FROM productos p inner join aten_prod ap on p.id=ap.fk_producto where p.id=".$this->idProducto;
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetch();
        Database::disconnect();
    ...
    function createProducto() {
        try {
            require_once "database.php";
            $pdo = Database::connect();
            $query = "insert into productos set nombre = ?,valor = ?,descripcion = ?,fk_categoria = ?";
            $stmt = $pdo->prepare($query);
    ...
    function updateProducto() {
            ...
            try {
                 require_once "database.php";
                 $pdo = Database::connect();
                 $query = "update productos set nombre = ?,valor = ?,descripcion = ?,fk_categoria = ? where id =".$this->idProducto;
                 $stmt = $pdo->prepare($query);
                 $stmt->bindParam(1, $this->nombre);
                 $stmt->bindParam(2, $this->valor);
                 $stmt->bindParam(3, $this->descripcion);
    ...
    function deleteProducto() {  
        if (!$this->productoAtenciones()) {
            ...
            return "*3* Error al tratar de eliminar Producto: El producto ya tiene atenciones registradas";
        }
    }
     /**
